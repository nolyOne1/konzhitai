...
“服务器—代理升级”集中显示各节点版本、系统架构和自升级能力。管理员可选择已导入版本与符合条件的节点创建计划：首批固定验证一台，成功后按设定批次自动推进；升级前会等待运行任务自然结束，不会强制中止脚本。任一节点失败会立即暂停计划，并只回滚失败批次，之前成功的批次保持目标版本。

旧代理未上报 `self_upgrade_v1` 时显示“需人工升级基线”，必须先按[部署手册](deploy/README.md#旧代理的一次性基线升级)完成一次人工更新。正常运行阶段可在计划详情中暂停、继续、取消、重试已回滚节点或对成功节点创建回滚计划；“需要人工处理”表示自动回滚也失败，应先在节点恢复服务与版本，再从控制台重试。
