...
包含迁移 14、15 的完整候选使用同一个 `migration apply` 命令。该路径只支持当前发布基线为版本 12，数据库为版本 12（首次执行）或版本 15（事务已提交后的恢复）。版本 13、14 的生产数据库会被拒绝。包含 16–19 的候选只使用下一节独立的 15→19 路径；既有仅包含版本 13 的候选仍使用上一节路径。
...
## 四、补充：版本 15 到 19 的平台功能发布

本路径与既有 12→13/15 候选分开。它只接受完整的 1–19 迁移树（每个版本恰好一对 up/down 文件），其中 16 为服务器分组、17 为脚本同步自动重试、18 为不限排队等待、19 为日志归档进度。当前成功发布记录的历史迁移摘要必须对应候选的 1–15 文件；部署契约及代理锁仍必须一致。版本 16、17、18 的中间数据库状态及含其他迁移的候选均拒绝执行。

操作前准备：

1. 核验候选运行、完整来源 SHA、来源证明与清单摘要，使用该候选内的发布程序和迁移树。
2. 准备数据库版本 **15** 的成功恢复点：本机与 COS 快照均成功，清单 SHA-256 有效，且独立恢复校验记录为成功、版本为 15。运维导出现在读取真实数据库版本；不能把写死为 12 的旧元数据当作版本 15 的证据。
3. 安排维护窗口，暂停配置变更、任务创建及新的派发，确认运行任务和代理升级已稳定。审批内容明确候选编号、源 SHA、恢复点与上一成功应用版本；获得具体生产授权后才执行第三节及第四节中的 `migration apply` 命令。

程序持有主机发布锁，在数据库事务中锁定迁移版本表并再次确认版本为 15，按序执行 16–19。迁移与不可变审计回执在同一事务提交；回执记录精确候选、源 SHA、前后迁移树摘要、恢复点及操作者。提交后核对分组回填、同步重试字段和索引、零等待约束、日志归档字段和索引，以及用户和有效会话数量；核对成功才写入该候选的发布基线。随后仍须通过正常应用发布审批和健康检查。

失败与恢复：

- **事务提交前失败**：整个事务回滚，仍保留版本 15，且不写发布基线。确认原因后用同一候选重试。
- **已提交版本 19，但校验或落盘中断**：用同一候选、源 SHA、恢复点和操作者重试。程序须匹配数据库中的精确回执，才能跳过 SQL并重新核验；不能使用另一份候选“补领”基线。
- **新应用健康检查失败**：标准发布流程可恢复上一成功应用镜像，数据库保留版本 19，不自动执行任何 down SQL。
- **稍后人工回滚应用**：先核查是否已有 `max_wait_seconds=0` 的新任务、活动日志归档与新增分组发布规则，评估旧应用行为，再选择已成功版本回滚。需要回到数据库版本 15 时，按独立灾难恢复流程恢复已验证恢复点；不能用逐个 down SQL删除新功能数据。

本节是发布程序的操作流程，不表示生产已完成迁移或升级。实际候选编号、执行时间、版本与验收证据只在真实执行后登记。
