...
仓库功能分支已实现多版本代理仓库、首批单节点验证、自动分批推进、失败批次回滚和全中文升级控制台，但本记录不把代码完成视为生产上线。生产仍按下方既有记录运行代理 `0.1.0`；迁移 14、受限升级 systemd 单元、新代理基线和控制台升级页面均须在候选发布审批、备份、隔离验证和真实节点验收完成后再填写实际时间、提交 SHA、镜像摘要与节点结果。
