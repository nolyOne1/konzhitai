...
仓库功能分支已实现多版本代理仓库、首批单节点验证、自动分批推进、失败批次回滚和全中文升级控制台，但本记录不把代码完成视为生产上线。生产仍按下方既有记录运行代理 `0.1.0`；迁移 14—15、受限升级 systemd 单元、新代理基线和控制台升级页面均须在候选发布审批、备份、隔离验证和真实节点验收完成后再填写实际时间、提交 SHA、镜像摘要与节点结果。
...
截至本文件本次代码更新，生产数据库仍按下文真实记录停在迁移版本 `12`；`000013_member_lifecycle` 未在生产执行，候选绑定的 `migration-baselines/<候选运行编号>.json` 也尚未生成。本仓库只准备了可执行机制和操作手册，没有授权或声称完成生产迁移、应用发布或验收。

真实执行前必须按 [迁移 13 受控 rollout](RELEASE.md#四成员生命周期迁移-13-的受控-rollout)完成独立审批、双份成功备份、迁移版本为 `12` 的隔离恢复核验和锁窗口判定。执行后仅根据实际证据补记：候选运行编号与完整源 SHA、恢复点 UUID、迁移基线摘要、执行时间、数据库版本、索引、既有登录/会话兼容性、应用发布结果和失败处置；任何“待执行”字段不得预填为成功。
