...
test('管理员创建成员并完成首次改密流程', async ({ page }) => {
  ...
  await page.getByLabel('姓名').fill('新值班运维')
  await page.getByLabel('邮箱').fill('new-ops@example.com')
  await page.getByRole('checkbox', { name: '运维人员' }).check()
  await page.getByRole('button', { name: '确认创建' }).click()
  const passwordDialog = page.getByRole('dialog', { name: '一次性临时密码' })
  await expect(passwordDialog).toBeVisible()
  const temporaryPassword = await passwordDialog.locator('code').innerText()
  await page.getByRole('button', { name: '我已保存，关闭' }).click()

  await page.reload()
  await expect(page.getByRole('button', { name: '创建成员' })).toBeVisible()
  await page.getByRole('button', { name: '退出登录' }).click()
  await expect(page).toHaveURL(/\/login$/)
  await page.getByLabel('邮箱').fill('new-ops@example.com')
  await page.getByLabel('密码').fill(temporaryPassword)
  await page.getByRole('button', { name: '登录', exact: true }).click()
  await expect(page).toHaveURL(/\/password-required$/)
  await expect(page.getByRole('heading', { name: '首次登录，请修改密码' })).toBeVisible()

  const protectedResponse = await page.evaluate(async () => {
    const response = await fetch('/api/members?status=all')
    return { status: response.status, body: await response.json() as { code?: string } }
  })
  expect(protectedResponse).toEqual({ status: 403, body: { code: 'password_change_required', message: '首次登录必须先修改密码' } })

  await page.getByLabel('当前密码').fill(temporaryPassword)
  await page.getByLabel('新密码', { exact: true }).fill('member-password-2026')
  await page.getByLabel('确认新密码').fill('member-password-2026')
  ...
  await page.getByLabel('密码').fill('member-password-2026')
  await page.getByRole('button', { name: '登录', exact: true }).click()
  await expect(page).toHaveURL(/\/$/)
})
...
test('管理员停用、移除、恢复成员并确认重置密码', async ({ page }) => {
  ...
  await expect(page.getByText('值班运维', { exact: true })).not.toBeVisible()
  await expect(page.getByRole('button', { name: '已移除' })).toBeFocused()
