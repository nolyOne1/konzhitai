...
describe('服务器管理', () => {
  ...
  it('展示资源、标签和排空操作，并可打开详情', async () => {
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (path === '/api/agent-releases') return { ok: true, status: 200, json: async () => ({ releases: [{ id: 'release-2', version: '0.2.0', status: 'available', recommended: true, release_notes: '稳定版', capabilities: ['self_upgrade_v1'], created_at: '2026-09-07T00:00:00Z', artifacts: [] }] }) } as Response
      if (path === '/api/auth/session') return { ok: true, status: 200, json: async () => ({ user: { user_id: 'admin-1', display_name: '管理员', email: 'admin@example.com', roles: ['admin'], must_change_password: false } }) } as Response
      return {
        ok: true, status: 200, json: async () => ({ servers: [{
          ...
          agentOS: 'linux',
          agentArch: 'amd64',
          agentCapabilities: ['self_upgrade_v1'],
    ...
    expect(screen.getByRole('navigation', { name: '服务器模块' })).toBeVisible()
    expect(screen.getByRole('link', { name: '节点管理' })).toHaveAttribute('href', '/servers')
    expect(screen.getByRole('link', { name: '代理升级' })).toHaveAttribute('href', '/servers/upgrades')
    expect(screen.getByRole('columnheader', { name: '代理版本' })).toBeVisible()
    expect(screen.getByText('可升级')).toBeVisible()
    ...
    expect(screen.getByText('支持控制台升级')).toBeVisible()
  })
  ...
  it('管理员可创建一次性注册令牌并复制新服务器安装命令', async () => {
    ...
    await user.type(within(dialog).getByLabelText(/服务器标签/), '用途=批处理, 环境=生产, __proto__=保留')
    ...
    expect(enrollmentInput).toEqual({ name: '阿里云执行节点-1', cloud_provider: '阿里云', region: '华东 1', labels: JSON.parse('{"用途":"批处理","环境":"生产","__proto__":"保留"}') })
    ...
    expect(within(closeConfirmation).getByRole('button', { name: '继续查看' })).toHaveFocus()
    await user.keyboard('{Shift>}{Tab}{/Shift}')
    expect(within(closeConfirmation).getByRole('button', { name: '确认关闭' })).toHaveFocus()
