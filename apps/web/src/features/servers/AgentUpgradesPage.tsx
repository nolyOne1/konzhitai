...
import { getAgentReleases, getAgentUpgradePlan, getAgentUpgradePlans, getServers, recommendAgentRelease, withdrawAgentRelease, type AgentRelease, type AgentUpgradePlan, type ServerView } from '../../api/client'
...
export function AgentUpgradesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextReleases, nextServers, nextPlans] = await Promise.all([getAgentReleases(), getServers(), getAgentUpgradePlans()])
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">多服务器代理运维</p><h1>代理升级</h1><p>集中盘点代理版本，首批单节点验证后自动分批升级。</p></div><button ref={createButtonRef} type="button" className="primary-action" disabled={Boolean(activePlan) || loading} onClick={() => setShowDialog(true)}>创建升级计划</button></div>
