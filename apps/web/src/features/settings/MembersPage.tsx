...
export function MembersPage() {
  ...
  const filterButtons = useRef<Partial<Record<MemberStatus, HTMLButtonElement | null>>>({})
  ...
  useEffect(() => {
    if (dialog !== null || !restoreFocus.current) return
    restoreFocus.current = false
    if (trigger.current?.isConnected) trigger.current.focus()
    else filterButtons.current[filter]?.focus()
  }, [dialog, filter, members])
  ...
  async function submitCreate(input: CreateMemberInput) {
    ...
    setMembers((current) => upsertMember(current, result.member, filter))
  ...
  async function saveRoles() {
    ...
    try {
      ...
      setMembers((current) => upsertMember(current, updated, filter))
...
function RoleDialog({ member, selected, saving, error, firstRole, onToggle, onSave, onClose }: { member: Member; selected: RoleName[]; saving: boolean; error: string; firstRole: RefObject<HTMLInputElement | null>; onToggle(role: RoleName): void; onSave(): void; onClose(): void }) {
  const dialogError = useRef<HTMLDivElement>(null)
  useEffect(() => { if (error) dialogError.current?.focus() }, [error])
  return <div className="drawer-backdrop centered-dialog"><section className="console-dialog" role="dialog" aria-modal="true" aria-labelledby="edit-role-title" onKeyDown={(event) => trapDialog(event, onClose)}><header className="drawer-header"><div><p className="eyebrow">最小权限原则</p><h2 id="edit-role-title">调整成员角色</h2><p>{member.displayName} · {member.email}</p></div><button className="icon-button" type="button" aria-label="关闭调整成员角色窗口" onClick={onClose}>×</button></header><div className="role-selector">{error ? <div ref={dialogError} className="form-error error-summary" role="alert" tabIndex={-1}>{error}</div> : null}{memberRoleOptions.map((role, index) => <label key={role.value} className="role-choice"><input ref={index === 0 ? firstRole : undefined} aria-label={role.label} type="checkbox" checked={selected.includes(role.value)} onChange={() => onToggle(role.value)} /><span><strong>{role.label}</strong><small>{role.description}</small></span></label>)}</div><footer className="dialog-actions"><button className="secondary-action" type="button" onClick={onClose}>取消</button><button className="primary-action" type="button" disabled={saving} onClick={onSave}>{saving ? '正在保存…' : '保存角色'}</button></footer></section></div>
}

function upsertMember(members: Member[], updated: Member, filter: MemberStatus) {
  const withoutUpdated = members.filter((member) => member.id !== updated.id)
  if (!matchesFilter(updated, filter)) return withoutUpdated
  const index = members.findIndex((member) => member.id === updated.id)
  if (index === -1) return [updated, ...members]
  return members.map((member) => member.id === updated.id ? updated : member)
}

function matchesFilter(member: Member, filter: MemberStatus) {
  if (filter === 'removed') return member.removedAt !== null
  if (member.removedAt !== null) return false
  if (filter === 'active') return member.enabled
  if (filter === 'disabled') return !member.enabled
  return true
}
